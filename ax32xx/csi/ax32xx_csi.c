...
void ax32xx_csiScaler(u16 src_w,u16 src_h,u16 tar_w,u16 tar_h,u16 sx,u16 sy,u16 ex,u16 ey)
{
	...
	ax32xx_ispModeSet(ISP_MODE_JPEGCROP,0); // disable csi to mjpeg crop
...
//		R_CSI_SCALER_CTRL = (1<<2)|1;
		...
		#if 0
		ax32xx_ispModeSet(ISP_MODE_JPEGCROP,1);     //enable csi to mjpeg crop
		#else
...
bool ax32xx_hvScaler(u32 src_w,  u32 src_h, u32 obj_w, u32 obj_h)
{
	...
	debg("wsacle,hsacle:%x,%x",wsacle,hsacle);
	{
		...
		//CSI_I2W_HCROP &= ~(3<<11);
		//CSI_I2W_VCROP &= ~(3<<10);
		ax32xx_ispModeSet(ISP_MODE_SDEN,0);
		ax32xx_ispModeSet(ISP_MODE_WIFICROP,0);
		...
		ax32xx_ispModeSet(ISP_MODE_SDEN,1);
