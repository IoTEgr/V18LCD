...
{
	drawString(&(state.errTipRect),&(state.errTipRect),id,0x02,ALIGNMENT_CENTER,0xfe,0xf8);
}
	...
	switch(msg->id)
	{
		case MSG_WIN_DESTROY:
			...
			while(hWin!=INVALID_HANDLE)
			{
				...
				pWin=(winObj*)uiHandleToPtr(hWin);
				hTemp=pWin->next;
				winSendMsgNoData(hWin,MSG_WIDGET_DESTROY);
				//if(pWin->name)
...
u32 winGetResMaxNum(winHandle hWin)
{
	winMsg msg;
...
void winSetResSum(winHandle hWin,u32 sum)
{
	winMsg msg;
...
void winSetResidByNum(winHandle hWin,u32 num,resID id,bool select)
{
	winMsg msg;
	resInfor infor;
	infor.id=id;
...
void winSetSelectStrInfor(winHandle hWin,charFont font,uint8 strAlign,uiColor fontColor)
{
	strInfor infor;
	winMsg msg;
	infor.font=font;
	infor.strAlign=strAlign;
	infor.fontColor=fontColor;
	msg.id=MSG_WIN_SELECT_STRINFOR;
	msg.parameter.p=&infor;
	winSendMsg(hWin,&msg);
}
