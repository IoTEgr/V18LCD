...
void dialogCB(winMsg* msg)
{
	...
	switch(msg->id)
	{
		...
		case MSG_WIN_PARENT_DEAL:
			...
			break;
		case MSG_WIN_TOUCH:
			if(widgetGetType(hSrc)==WIDGET_ITEM_MANAGE)
			{
				msg->parameter.p=(void*)itemManageGetResNum(hSrc,(uiRect*)msg->parameter.p,argv1);
				if(argv1[0])
					msgDealByType(SYS_TOUCH_WINDOW,hWin,argv1[0],&argv1[1]);
			}
			else
			{
				argv1[0]=1;
				argv1[1]=widgetGetId(hSrc);
				msgDealByType(SYS_TOUCH_WINDOW,hWin,argv1[0],&argv1[1]);
				msg->parameter.p=(void*)hSrc;
			}
			break;
		default:
			break;
