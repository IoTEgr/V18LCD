...
static int formatKeyMsgPower(winHandle handle,uint32 parameNum,uint32* parame)
{
	...
	return formatKeyMsgOk(handle, parameNum, parame);
}

static int formatKeyMsgEsc(winHandle handle,uint32 parameNum,uint32* parame)
{
	return formatKeyMsgMenu(handle, parameNum, parame);
}

#if BAT_FLOW_CHARGE_EN
static int formatKeyMsg1s(winHandle handle,uint32 parameNum,uint32* parame)
{
	if(SysCtrl.usb != NULL)
	{
...
msgDealInfor formatMsgDeal[]=
{
	...
	{KEY_EVENT_POWER,formatKeyMsgPower},
	{KEY_EVENT_ESC,formatKeyMsgEsc},
#if BAT_FLOW_CHARGE_EN
	{SYS_EVENT_1S,formatKeyMsg1s},
#endif
	{EVENT_MAX,NULL},
};
