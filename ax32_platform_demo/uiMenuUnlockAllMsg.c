...
static int unlockAllKeyMsgPower(winHandle handle,uint32 parameNum,uint32* parame)
{
	...
	return unlockAllKeyMsgOk(handle, parameNum, parame);
}

static int unlockAllKeyMsgEsc(winHandle handle,uint32 parameNum,uint32* parame)
{
	return unlockAllKeyMsgMenu(handle, parameNum, parame);
}
...
msgDealInfor unlockAllMsgDeal[]=
{
	...
	{KEY_EVENT_ESC,unlockAllKeyMsgEsc},
	{EVENT_MAX,NULL},
};
