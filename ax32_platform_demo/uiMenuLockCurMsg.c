...
static int lockCurKeyMsgPower(winHandle handle,uint32 parameNum,uint32* parame)
{
	...
	return lockCurKeyMsgOk(handle, parameNum, parame);
}

static int lockCurKeyMsgEsc(winHandle handle,uint32 parameNum,uint32* parame)
{
	return lockCurKeyMsgMenu(handle, parameNum, parame);
}
...
msgDealInfor lockCurMsgDeal[]=
{
	...
	{KEY_EVENT_POWER,lockCurKeyMsgPower},
	{KEY_EVENT_ESC,lockCurKeyMsgEsc},
