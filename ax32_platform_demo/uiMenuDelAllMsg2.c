...
static int delAllKeyMsgOk2(winHandle handle,uint32 parameNum,uint32* parame)
{
	...
	if(keyState==KEY_PRESSED)
	{
		...
		if(item==0)
		{
			...
			int cnt_wav = managerFileCount(SysCtrl.wav_list);
			for(i=0;i<cnt_wav;i++)
			{
				name = manangerGetFileFullPathName(SysCtrl.wav_list,i+1,NULL);
				deg_Printf("delete : %s.",name);
				if(f_unlink(name)==FR_OK)
					deg_Printf("->ok\n");
				else
			...
			managerDelFileAll(SysCtrl.wav_list); //del wav listnode

			SysCtrl.file_index=0;//managerFileCount(SysCtrl.avi_list);
		}
		else
			winDestroy(&handle);
	}
...
static int delAllKeyMsgpPower2(winHandle handle,uint32 parameNum,uint32* parame)
{
	...
	return delAllKeyMsgOk2(handle, parameNum, parame);

}

static int delAllKeyMsgpEsc(winHandle handle,uint32 parameNum,uint32* parame)
{
	return delAllKeyMsgMenu(handle, parameNum, parame);

}

static int delAllOpenWin2(winHandle handle,uint32 parameNum,uint32* parame)
{
...
msgDealInfor delAllMsgDeal2[]=
{
	{SYS_OPEN_WINDOW,delAllOpenWin2},
	{SYS_CLOSE_WINDOW,delAllCloseWin},
	{SYS_CHILE_COLSE,delAllWinChildClose},
	{KEY_EVENT_OK,delAllKeyMsgOk2},
	{KEY_EVENT_UP,delAllKeyMsgUp},
	{KEY_EVENT_DOWN,delAllKeyMsgDown},
	//{KEY_EVENT_MENU,delAllKeyMsgMenu},
	{KEY_EVENT_MODE,delAllKeyMsgMode},	
	{KEY_EVENT_POWER,delAllKeyMsgpPower2},
	{KEY_EVENT_ESC,delAllKeyMsgpEsc},
	{EVENT_MAX,NULL},
};

WINDOW(delAllWindow2,delAllMsgDeal2,delAllWin2)
