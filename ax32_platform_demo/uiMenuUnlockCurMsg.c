...
static int unlockCurKeyMsgPower(winHandle handle,uint32 parameNum,uint32* parame)
{
	...
	return unlockCurKeyMsgOk(handle, parameNum, parame);
}

static int unlockCurKeyMsgEsc(winHandle handle,uint32 parameNum,uint32* parame)
{
	return unlockCurKeyMsgMenu(handle, parameNum, parame);
}
...
msgDealInfor unlockCurMsgDeal[]=
{
	...
	{KEY_EVENT_POWER,unlockCurKeyMsgPower},
	{KEY_EVENT_ESC,unlockCurKeyMsgEsc},
	{EVENT_MAX,NULL},
};
