...
static int dateTimeKeyMsgPower(winHandle handle,uint32 parameNum,uint32* parame)
{
	...
	return dateTimeKeyMsgOk(handle, parameNum, parame);

}

static int dateTimeKeyMsgEsc(winHandle handle,uint32 parameNum,uint32* parame)
{
	return dateTimeKeyMsgMenu(handle, parameNum, parame);
...
msgDealInfor dateTimeMsgDeal[]=
{
	...
	{KEY_EVENT_ESC,dateTimeKeyMsgEsc},
	{EVENT_MAX,NULL},
};
