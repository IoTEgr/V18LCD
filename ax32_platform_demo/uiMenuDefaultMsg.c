...
static int defaultKeyMsgPower(winHandle handle,uint32 parameNum,uint32* parame)
{
	...
	return defaultKeyMsgOk(handle, parameNum, parame);
}

static int defaultKeyMsgEsc(winHandle handle,uint32 parameNum,uint32* parame)
{
	return defaultKeyMsgMenu(handle, parameNum, parame);
}
...
msgDealInfor defaultMsgDeal[]=
{
	...
	{KEY_EVENT_ESC,defaultKeyMsgEsc},
	{EVENT_MAX,NULL},
};
