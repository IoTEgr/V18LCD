...
#if HAL_CFG_LCD_CROP
/****************************************
sx=160, sy=120, ex=480, ey=360
sx=128, sy=96, ex=512, ey=384
sx=96, sy=72, ex=544, ey=408
sx=64, sy=48, ex=576, ey=432
sx=32, sy=24, ex=608, ey=456   position
sx=0, sy=0, ex=640, ey=480     calculate
*****************************************/
int dispLayerSetFrontCrop_426_240(INT8U level)
{
    static const u32 w_h[][2]={{426,240} /*{320,240}*/ , {384,288},{448,336},{512,384},{576,432},{640,480}};
    
    u16 csi_w,csi_h,crop_w,crop_h;
    u16 sx,sy,ex,ey;
    
    hal_csiResolutionGet(&csi_w,&csi_h);
    crop_w = w_h[0][0];debgreg(crop_w);
    crop_h = w_h[0][1];debgreg(crop_h);
    sx = (csi_w - crop_w) / 2;
    ex = sx + crop_w;
    sy = (csi_h - crop_h) / 2;
    ey = sy + crop_h;
