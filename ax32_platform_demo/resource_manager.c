...
#if MP3_DECODE_SURPORT
typedef struct MP3_NAME_LIST_S
{
	unsigned char name_list[MANAGER_LIST_MAX][32+MANAGER_NAME_LEN+1];
}MP3_NAME_LIST_T;

MP3_NAME_LIST_T *mp3_name_list = NULL;
...
int mp3_name_list_init(void)
{
	if(mp3_name_list)
		hal_sysMemFree(mp3_name_list);
	mp3_name_list = hal_sysMemMalloc(sizeof(MP3_NAME_LIST_T),64);
	if(mp3_name_list == NULL)
	{
		deg_Printf("mp3 name list malloc fail\n");
		return -1;
	}
	deg_Printf("mp3 name list malloc ok.\n");
	return 0;
}
void mp3_name_list_unit(void)
{
	if(mp3_name_list)
		hal_sysMemFree(mp3_name_list);
	mp3_name_list = NULL;
	deg_Printf("mp3 name list uint ok.\n");
}
...
static void namemake(char *path,char *str,RES_NAME_T *mname)
{
	...
	if(strncmp(str,PREFIX_LOK,3)==0)
	{
			...
			if(MA_FILE_AVIA == (mname->index_h & MA_FILE_AVIA))
			{
				mname->index_h |= MA_FILE_LOKA;
			}
			else if(MA_FILE_AVIB == (mname->index_h & MA_FILE_AVIB))
			{
				mname->index_h |= MA_FILE_LOKB;
			}
			else
			{
...
int managerLockFile(char *name)
{
    ...
    if((i==0)&&(name[0] != '/'))
		...
		if(i > 0)
		{
			//deg_Printf("i=%d.type=%d,%c\n",i,type,name[i-2]);
			if(FILE_TYPE_AVI == type)
			{
				u8 temp_name[16];
				...
				if('A' == name[i-2])	
				{
					...
					strcpy(name,FILEDIR_LOCKA);
					strcpy(name+strlen(FILEDIR_LOCKA),temp_name);
					i = strlen(FILEDIR_LOCKA);
				}
				else if('B' == name[i-2])
				{
					strcpy(temp_name,&name[i]);
					strcpy(name,FILEDIR_LOCKB);
					strcpy(name+strlen(FILEDIR_LOCKB),temp_name);
					i = strlen(FILEDIR_LOCKB);
				}
			}
		}
	#endif
	...
	if((i==0)&&(name[0] != '/'))
		...
		if(i > 0)
		{
			if(FILE_TYPE_AVI == type)
			{
				...
				{
					strcpy(temp_name,&name[i]);
					strcpy(name,FILEDIR_VIDEOA);
					strcpy(name+strlen(FILEDIR_VIDEOA),temp_name);
					i = strlen(FILEDIR_VIDEOA);
				}
				else if('B' == name[i-2])
				{
					strcpy(temp_name,&name[i]);
					strcpy(name,FILEDIR_VIDEOB);
					strcpy(name+strlen(FILEDIR_VIDEOB),temp_name);
					i = strlen(FILEDIR_VIDEOB);
				}
			}
		}
	#endif
	...
	{         
		 ...
		 if(finfo.fname[0] != '.')
		 {
			 ...
			 else
			 {
				 ...
				 if((FILE_TYPE_WAV == type)&&(strncmp(dir,FILEDIR_AUDIO,6) == 0))
				 {
					 //deg_Printf("mp3 file\n");
					 tname.index_h = MA_FILE_WAV;
					 tname.index_l = resManager[exp].count;
					 
					 strcpy(mp3_name_list->name_list[tname.index_l],finfo.fname);
					 //debgbuf(mp3_name_list[tname.index_l],sizeof(mp3_name_list[tname.index_l]));
					// deg_Printf("-------mp3_name_list->name_list=%s-------.\n",&mp3_name_list->name_list[tname.index_l] );
				 }
				 else
				 #endif
			 	{
				...
				//deg_Printf("-------finfo.fname=%s---tname.index_h=0x%x,tname.index_l=0x%x----.\n",finfo.fname,tname.index_h,tname.index_l);
					
				 resManager[exp].count++;
			#if 0//(MANAGER_NAME&MANAGER_NAME_INDEX)
				 if((resManager[exp])<(tname.index_l))
				 {
				 	resManager[exp] = tname.index_l;
				 }
			#endif	 
	             if(manager->add(resManager[exp].exp,&tname)<0)
				 	deg_Printf("e");
...
int managerNameChange(int exp,RES_NAME_T *rname,char *name,int path)
{
    ...
    if(path)
    {
		...
		{
			#if (1 == AVI_LOCK_FOLDER)
				if((rname->index_h&MA_FILE_LOKA) == MA_FILE_LOKA)
				 	strcat(name,FILEDIR_LOCKA);
				else if((rname->index_h&MA_FILE_LOKB) == MA_FILE_LOKB)
				    strcat(name,FILEDIR_LOCKB);
				else if((rname->index_h&MA_FILE_AVI) == MA_FILE_AVIA)
...
#if (1 == MP3_DECODE_SURPORT)
		if(rname->index_h&MA_FILE_WAV)
		{
			strcpy(&name[len],mp3_name_list->name_list[rname->index_l]);
			//debgbuf(name,sizeof(name));
			type = MA_FILE_WAV;
			return type;
		}
		/*else if(rname->index_h&MA_FILE_DIR)
		{
			strcpy(&name[len],mp3_name_list->name_list[rname->index_l]);
			type = MA_FILE_DIR;
			return type;
