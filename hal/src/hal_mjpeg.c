...
//			crop_w = 320;
//			crop_h = 320;
			crop_w = (csi_w * cropLevels / 100) & ~1;
			crop_h = (csi_h * cropLevels / 100) & ~1;
			sx = (csi_w - crop_w) / 2;
			ex = sx + crop_w;
			sy = (csi_h - crop_h) / 2;
			ey = sy + crop_h;
			...
			ax32xx_csiMJPEGFrameSet(mjpegEncCtrl.ybuffer,mjpegEncCtrl.uvbuffer,HAL_CFG_MJPEG_HEIGHT,HAL_CFG_MJPEG_WIDTH);
#else		
		ax32xx_csiScaler(mjpegEncCtrl.csi_width,mjpegEncCtrl.csi_height, win_w, win_h,0,0,mjpegEncCtrl.csi_width, mjpegEncCtrl.csi_height);
...
void hal_mjpegAVISetScaler(u16 crop_sx,u16 crop_sy,u16 crop_ex,u16 crop_ey)
{
	u16 csi_w,csi_h,crop_w,crop_h;
	...
	crop_w = (csi_w * cropLevels / 100) & ~1;
	crop_h = (csi_h * cropLevels / 100) & ~1;
	ax32xx_csiScaler(csi_w,csi_h, crop_w,crop_h,crop_sx,crop_sy,crop_ex,crop_ey);
	ax32xx_mjpegEncodeSizeSet(crop_w,crop_h,mjpegEncCtrl.mjpeg_width,mjpegEncCtrl.mjpeg_height);
