...
void  *hal_watermarkRam(s32 layer,INT16U *width,INT16U *height)
{
    ...
    //deg_Printf("water_mark, layer=%x, res=%x, r1= %x, r2= %x\r\n", layer, configGet(CONFIG_ID_RESOLUTION), R_ID_STR_RES_HD, R_ID_STR_RES_FHD);
    if(configGet(CONFIG_ID_RESOLUTION) == R_ID_STR_RES_FHD)
    {
        layer = 2;
    }
    else
    ...
    deg_Printf("water_mark, r1= %d, r2= %d\r\n", hal_watermark_ctrl[layer].width, hal_watermark_ctrl[layer].height);
