...
typedef struct _lcd_config_s 
{
    ...
    lcddev_t *  p_lcddev;
#if HAL_CFG_LCD_CROP
	u16 crop_sx,crop_ex,crop_sy,crop_ey, crop_w, crop_h;
#else
	// for LDMA crop
...
static u8* lcd_temp_decode_buff;
static u32 decode_buff_stride,decode_buff_h;

static imageCopyRect display_region;
#endif
...
#if HAL_CFG_LCD_CROP
        lcd_config.crop_w  = p_lcddev->video_w;
        lcd_config.crop_h  = p_lcddev->video_h;
#endif
...
#if HAL_CFG_LCD_CROP
    decode_buff_stride = HAL_CFG_LCD_CROP_W;
    decode_buff_h = HAL_CFG_LCD_CROP_H;
    deg_Printf("[%s]  stride== %d\n",__FUNCTION__,decode_buff_stride);
    lcd_temp_decode_buff = hal_sysMemMalloc(decode_buff_stride*decode_buff_h*3/2, 64);//ALIGN 64
    if(lcd_temp_decode_buff == NULL)
    {
        deg_Printf("malloc mem fail-------------->\r\n");
    }
#endif
...
#if HAL_CFG_LCD_CROP
    deg_Printf("bak w=%d, h=%d \r\n", lcd_config.csi_using->w, lcd_config.csi_using->h);
#endif
    //TODO:能否沿用csi_using?
...
* Return         : None
*******************************************************************************/
static void hal_lcdDealBuffer(disp_frame_t * p_lcd_buffer)
{
...
void hal_lcdSwitchFrame2(void)
{
    ...
    if(isp_vch->config_enable)
    {
		...
		//csi_w = csi_h;
#if HAL_CFG_LCD_CROP //#if 0 //TEST_FLAG_XXX
		if(1)
		{//426*240 --> 240:240
			ax32xx_csiLCDScaler(csi_w,csi_h,
								...
								0,0,csi_w,csi_h,
					#else
								lcd_config.crop_sx+(lcd_config.crop_ex - lcd_config.crop_ey)/2,
								lcd_config.crop_sy,
								lcd_config.crop_ex-(lcd_config.crop_ex - lcd_config.crop_ey)/2,
								lcd_config.crop_ey,
					#endif
								ready->stride,
								(u32*)lcd_config.p_lcddev->lcd_lsawtooth.anti_lsawtooth);
                                deg_Printf("aaa sx=%d, sy=%d, ex=%d, ey=%d\r\n", lcd_config.crop_sx, lcd_config.crop_sy, lcd_config.crop_ex, lcd_config.crop_ey);

            u8 * p_y = &lcd_config.csi_using->y_addr[jpg_vch->x + (jpg_vch->y * lcd_config.csi_using->stride)];
            u8 * p_uv= &lcd_config.csi_using->uv_addr[jpg_vch->x + (jpg_vch->y / 2 * lcd_config.csi_using->stride)];
            //deg_Printf("999 w=%d, h=%d\r\n", lcd_config.csi_using->w, lcd_config.csi_using->h);

            display_region.destX=0;
            display_region.destY=0;
            display_region.srcX=(lcd_config.crop_w-lcd_config.displaying->w)>>1;//0;//
            display_region.srcY=(lcd_config.crop_h-lcd_config.displaying->h)>>1;//0;//
            display_region.w=lcd_config.displaying->w;
            display_region.h=lcd_config.displaying->h;
            //deg_Printf("999 w=%d, h=%d\r\n", lcd_config.csi_using->w, lcd_config.csi_using->h);
            //lcd_temp_decode_buff = p_uv; //copy from 426x240 src buff
		}
#else
		if(1)
 		{//(1)960*720 -- 4:3 //(2)864:720(crop_xx=208, crop_xY=1072) -- 12:10// (3)23:20 828(1054-226) -- (4)720*720 -- 1:1
			u16 crop_xx, crop_xy;
			crop_xx = lcd_config.crop_sx+(lcd_config.crop_ex - lcd_config.crop_ey*23/20)/2; //lcd_config.crop_sx+(lcd_config.crop_ex - lcd_config.crop_ey*4/3)/2;
			crop_xy = lcd_config.crop_ex-(lcd_config.crop_ex - lcd_config.crop_ey*23/20)/2;
			...
			ax32xx_csiLCDScaler(csi_w,csi_h,
								...
								ready->stride,
								(u32*)lcd_config.p_lcddev->lcd_lsawtooth.anti_lsawtooth);
			//deg_Printf("xxxxx crop_xx=%d, crop_xY=%d\r\n", crop_xx, crop_xy);
		}
#endif
		else
		{
		...
		//ENTER();deg_Printf("sx=%d, sy=%d, ex=%d, ey=%d\r\n", lcd_config.crop_sx, lcd_config.crop_sy, lcd_config.crop_ex, lcd_config.crop_ey);
	   //deg_Printf("ax=%d, ay=%d, bx=%d, by=%d, ready->stride=%d\r\n", csi_w, csi_h, isp_vch->w, isp_vch->h, ready->stride);
    }
    ...
    if(((ready->pip_sta & buf_ready) == buf_ready) && not_skip)
    {
        ...
        {
        #if HAL_CFG_LCD_CROP
            imageRegionCopy(ready->y_addr, HAL_CFG_LCD_CROP_H, HAL_CFG_LCD_CROP_H,
                lcd_temp_decode_buff, HAL_CFG_LCD_CROP_W, HAL_CFG_LCD_CROP_H , &display_region);
        #endif
...
{
    deg_Printf("crop, x0=%d, y0=%d, x1=%d, y1=%d\r\n", crop_sx, crop_sy, crop_ex, crop_ey);
...
#if HAL_CFG_LCD_CROP
    lcd_config.crop_w = crop_ex - crop_sx;
    lcd_config.crop_h = crop_ey - crop_sy;
    deg_Printf("[1412 csicrop()]crop_w=%d, crop_h=%d\r\n", lcd_config.crop_w, lcd_config.crop_h);
#else
...
s32 hal_lcdSetShowingSize_rename(u16 srcw,u16 srch,u16 width,u16 height)
{
	u16 csi_w,csi_h;
	hal_csiResolutionGet(&csi_w,&csi_h);
