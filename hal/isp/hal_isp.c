...
extern u8 get_info_value(u8 *name);
void isp_LSC_adj_cfg(u16 *p_lsc_tab,u8 first_time){
	_Sensor_Ident_ * p_sen_ident  = hal_csi_SeninitGet();
	if(first_time)
	{
		if(get_info_value("sensor"))
		{
			 //hal_spiFlashRead(SEN_RES_FLASH_ADDR(p_sen_ident->lsc_tab_adr),(u32)p_lsc_tab,p_sen_ident->lsc_tab_size);
			 //debg("show sensor value[%d] use first\n",get_info_value("sensor"));

			hal_spiFlashRead(SEN_RES_FLASH_ADDR(p_sen_ident->lsc_tab_adr_rotato),(u32)p_lsc_tab,p_sen_ident->lsc_tab_size);
			debg("show sensor value[%d] use second\n",get_info_value("sensor"));
		}
		else
		{
			 //hal_spiFlashRead(SEN_RES_FLASH_ADDR(p_sen_ident->lsc_tab_adr_rotato),(u32)p_lsc_tab,p_sen_ident->lsc_tab_size);
	...
	if((_GET_ISP_MODE_(*isp.p_all_mod,_RGB_GAMA_POS_) == _ISP_AUTO_)){
	    ...
	    //fog_y_low = clip(fog_y_low,Y_GMA[temp]/4,255) - (Y_GMA[temp]/4 - Y_GMA[temp - 3]/4); 
		  if(Br>24)
            ...
            fog_y_low=Br;
		isp.p_rgb_gama->rmin = isp.p_rgb_gama->gmin = isp.p_rgb_gama->bmin = fog_y_low; 
...
void isp_calculate(STAT *pstat,u8 Br)
{
	...
	pstat-> ylog_buf[isp.p_ae->exp_adapt.ylog_cal_fnum-1] = (pstat->ysum *8 +high_sum*2 +low_sum*0)/ (pstat-> ptsum*8 + high_dot*2 + low_dot*0);
	pstat->yloga_avg = 0;
...
void isp_adjust(STAT *pstat,u32 *p_Br){
	...
	if(_GET_ISP_MODE_(*isp.p_all_mod,_CCM_POS_) == _ISP_AUTO_)
	{
		if( !hal_gpioRead(GPIO_PB,GPIO_PIN6) )// need ir
		{
			u16 ccm[9]={86,85,85,85,86,85,85,85,86};
			isp_CCM_cfg( ccm,isp.p_ccm->s41,isp.p_ccm->s42,isp.p_ccm->s43);
		}
		else 
		{
			isp_CCM_cfg( isp.p_ccm->ccm,isp.p_ccm->s41,isp.p_ccm->s42,isp.p_ccm->s43);
	...
	if(_GET_ISP_MODE_(*isp.p_all_mod,_EE_POS_) == _ISP_AUTO_){//ee ????????  BR
		...
		u8 ee_dn_slope;
        u8 ee_sharp_slope;
        static u8 last_ee_dn_slope=200;
        if(sen_gain<=512)
        {
            ee_dn_slope=0;
            ee_sharp_slope=1;
        }
        else if(sen_gain>=p_exp_adapt->gain_max)
        {
            ee_dn_slope=3;
            ee_sharp_slope=4;
        }
        ...
        {
            ee_dn_slope=((sen_gain-512)*(3))/(p_exp_adapt->gain_max-512);
            ee_sharp_slope=1+((sen_gain-512)*(3))/(p_exp_adapt->gain_max-512);
        }
        if(last_ee_dn_slope!=ee_dn_slope)
        {
            last_ee_dn_slope=ee_dn_slope;
            isp_EE_cfg(0,ee_dn_slope,ee_sharp_slope,0,0,0);
        }
	}
...
void sensor_rotate_adapt(u32 frame_rate)
{

	_Sensor_Adpt_ *csiSensor = hal_csiAdptGet();

	csiSensor->rotate_adapt.rotate=frame_rate;
	csiSensor->p_fun_adapt.fp_rotate(csiSensor->rotate_adapt.rotate);
}
...
bool exposal_adj(u32 yloga_avg,u32 yloga,u32 *p_br,u32 exp_ext_mod)
{
	...
	*p_br = exp_br_cal(yloga ,p_exp_adapt->exp_gain,p_exp_adapt->k_br); 
	...
	if(common_adapt.exp_gain_en){
		common_adapt.exp_gain = clip(common_adapt.exp_gain,p_exp_adapt->exp_min*256,p_exp_adapt->gain_max*step_val*step_max);
...
void ispReadFlash(u8 *pbdata ,u32 trans_len, u32 param_3B)
{
	hal_spiFlashRead(param_3B,(u32) pbdata,trans_len);
}
