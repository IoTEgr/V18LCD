...
const static INT8S videoSpeedTable[9] = {-4,-3,-2,-1,0,1,2,3,4};  // speed time by second
...
static int video_avi_read(void *buffer,INT32U offset,INT32U len)
{
	...
	lseek(videoPCtrl.arg.media.src.fd,offset,0);
...
int videoPlaybackStart(VIDEO_PARG_T *arg)
{
    ...
    jpgcachesize = hal_sysMemRemain()-8192;
...
//-------cache jpeg data
        ...
        if(err == X_ERR_NONE)
        {
			...
			if((videoPCtrl.oldframe == offset)|| (0 == length)) // same frame
			{	
				//videoPCtrl.PlayFrames++;
				XMsgQPost(videoPCtrl.jpegIdleQ,(MSG_T *)jpeg);
...
SERVICE_VIDEO_DECODE:
	...
	//-------register	
		...
		if(videoPCtrl.dacstat == 0x10) 
		{
			...
			//XMsgQPost(videoPCtrl.vidsBusyQ,(MSG_T *)p_lcd_uffer);
